...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"build_sample_pcaps: wrote {normal_path}  packets={len(normal_pkts)}  bytes={normal_size}"
    )
    print(
        f"build_sample_pcaps: wrote {attack_path}  packets={len(attack_pkts)}  bytes={attack_size}"
    )
